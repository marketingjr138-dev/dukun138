/**
 * DUKUN138 GUIDE PWA v1.7.2 - Google Apps Script Sync
...
function doGet(e) {
  ...
  if (action === 'ping') return jsonResponse({ ok: true, message: 'DUKUN138 GUIDE GAS Sync aktif', time: new Date().toISOString() });
...
function doPost(e) {
  try {
    ...
    if (body.action === 'saveConfig') {
      ...
      logAction('saveConfig', 'ok', 'Config updated from PWA v1.7.2');
...
function configToRows(config) {
  return [
    ['version', config.version || '1.7.0', 'versi config'],
